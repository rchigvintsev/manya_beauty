class Admin::CommentsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @comment.update(comment_params)
        ...
        @comment.save
        format.html { redirect_to admin_comment_url(@comment, page: current_page),
                                  notice: I18n.translate('comment.flash.actions.update.notice') }
        format.json { render :show, status: :ok, location: admin_comment_url(@comment, page: current_page) }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      ...
      format.html { redirect_to admin_comments_url(page: page),
  ...
  def publish
    respond_to do |format|
      if @comment.published
        ...
        format.html { redirect_to admin_comments_url(page: current_page), flash: {warn: warning} }
        ...
        if @comment.save
          format.html { redirect_to admin_comments_url(page: current_page),
                                    notice: I18n.translate('comment.flash.actions.publish.notice') }
          format.json { render :index, status: :ok, location: admin_comments_url(page: current_page) }
        else
          alert = I18n.translate('comment.flash.actions.publish.alert',
                                 errors: @comment.errors.full_messages.to_sentence)

          format.html { redirect_to admin_comments_url(page: current_page), alert: alert }
          format.json { render json: @comment.errors, status: :unprocessable_entity }
        end
      end
    end
